...
const GRAMMARS: GrammarSource[] = [
    ...
    // The JavaScript grammar (`source.js`) is intentionally NOT downloaded. A
    // minimal placeholder lives at `src/grammars/js.tmLanguage.json` to keep the
    // bundle small, since embedded JS highlighting is rarely used. To restore
    // full JS highlighting, re-add an entry here pointing at the upstream
    // JavaScript.tmLanguage.json and run `pnpm update-grammars`.
];
