...
import {
    ...
    setEditorValue,
} from '../src/init-editor';
import { WasmLoadError } from '../src/lib/errors';
import { RegistryManager, type WasmSource } from '../src/lib/registry';
...
// coordinates. An empty rect list makes them fall back to line-based
// movement instead of throwing.
 ...
 *
 * @param target Element to dispatch the event on, e.g. the editor content or
 * an input inside the search panel.
 * @param key The letter or character pressed, e.g. `f` or `/`.
 * @param mods Modifier keys held down; `ctrl` defaults to `true`.
...
const pressCtrlKey = (view: EditorView, key: string): KeyboardEvent => {
    return pressKey(view.contentDOM, key);
};
...
const mountEditor = async (
    ...
    return initEditor(textarea, wasmSource, {
        highlight: 'none',
        ...conf,
...
test('mounts an editor and highlights a comment', async () => {
    const view = await mountEditor({ highlight: 'full', hotkeys: { mode: 'mac' } }, wasm);
...
test("highlight: 'none' mounts without loading WASM", async () => {
    ...
    const view = await mountEditor({ hotkeys: { mode: 'mac' } });
    view.dispatch({ changes: { from: 0, insert: '! comment' } });
...
test("highlight: 'full' without a WASM source throws WasmLoadError", async () => {
    await expect(
        mountEditor({ highlight: 'full', hotkeys: { mode: 'mac' } }),
...
test('does not steal focus when autofocus is disabled', async () => {
    ...
    const view = await mountEditor({ autofocus: false });
...
test('Ctrl+Alt+F opens the search panel with the focus in the replace field', async () => {
    ...
    const event = pressKey(view.contentDOM, 'f', { alt: true });
...
test('Ctrl+S triggers onSave while the search panel has focus', async () => {
    const onSave = vi.fn();
    const view = await mountEditor({ hotkeys: { onSave } });
    ...
    const event = pressKey(findField!, 's');
...
test('Ctrl+/ toggles comments while the search panel has focus', async () => {
    ...
    const event = pressKey(findField!, '/');
...
test('Ctrl+L opens the go-to-line dialog', async () => {
    ...
    const event = pressKey(view.contentDOM, 'l');
...
test('Ctrl+D deletes the current line', async () => {
    ...
    const event = pressKey(view.contentDOM, 'd');
