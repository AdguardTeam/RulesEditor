...
- **Key handlers always consume the key** — a `run` that returns `false`
  falls through to the next binding on the same chord, which may be a
  destructive default (`Shift-Mod-k` is `deleteLine`). Return `true` even
  when the action is a no-op (`preventDefault` alone does not stop the
  fall-through), pin the no-match/no-op path with a test, and fall back to
  a sensible target (e.g. the find field when the replace field is
  missing) instead of consuming the key without any effect.
...
- **Document precedence** — built-in keymaps are registered before
  `conf.extensions`, so a consumer binding cannot shadow them; document the
  precedence and the `Prec.high(...)` escape hatch in the README rather
  than reordering the built-ins, which would silently change behavior for
  every consumer.
...
- CodeMirror resolves the platform once, when `@codemirror/view` is first
  ...
  `test/hot-keys-platform-windows.test.ts`) that stubs `navigator.platform`
...
- **Every documented config option must be real** — an option kept for
  ...
  in its JSDoc and in the README table (e.g. `conf.hotkeys.mode` is a
  no-op); the docs must not imply behavior the code does not implement.
